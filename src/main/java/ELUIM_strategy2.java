...
Generator3 generator;
    ...
    /* Generate a 64-bit fingerprint as int[], with negligible collision risk (≈0) */
    // For demonstration only; use MurmurHash3 64 in practice
    // A better 64-bit fingerprint (no external libraries version)
...
//        res is MaxDisConSet
    ...
    public void LUM(String delPre,int[] itemset) throws IOException {
        ...
        Pattern pattern;
        if(delPre==null){
            pattern = generator.getPattern3(itemset);
        }
        else{
            String[] delPreArr=delPre.split("&");
            int lastItem=Integer.valueOf(delPreArr[delPreArr.length-1]);
            pattern = generator.getPattern4(itemset,lastItem);
        }
        // twoListContainer represents the total utility of the pattern and each item within the pattern
