...
public class ULB {
    ...
    public int getLBofItemsetandItem(int lastDelItem,int itemIndex){
        if(itemset[itemIndex]<=lastDelItem){ return 0;}
        int LB=0;
        for (int i = 0; i < itemset.length; i++) {
            if (itemset[i]>lastDelItem){
                break;
        ...
        LB+=mapItemToLB.get(itemset[itemIndex]);
        return LB;
    }
    ...
    public int getLBofExW2(int lastDelItem, List<Integer> deleteList){
